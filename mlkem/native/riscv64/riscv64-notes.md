...
Experimental flags are enabled with `OPT=5` in development phase.
...
a30184edee53b3b009356e1e31d7f9e93ce82550e3c622d7192e387b0cc84f2e
```
That's a match (with the hash in `META.yml`).

Note: In case keccak instruction is enabled in spike (below) and in the build 
(`CFLAGS += -DMLK_SYS_RISCV64_KECCAK_INST -DMLK_CONFIG_USE_NATIVE_BACKEND_FIPS202`),
one needs to invoke spike with `zvkk`:
```
spike --isa=rv64gcv_zbb_zvbb_zvl256b_zvkk_zicntr_zihpm pk -p test/build/mlkem512/bin/gen_NISTKAT512  | sha256sum
```
